...
void main() {
  ...
  group('RegisterScreen', () {
    testWidgets('рендерит username, email, password и submit', (tester) async {
    ...
    testWidgets(
      'короткий username → submit не зовёт VM, errorText появляется',
      (tester) async {
        await tester.pumpWidget(
          _harness(loginVm: loginVm, registerVm: registerVm),
        );
    ...
    testWidgets('валидные данные → VM.submit вызван с trim()', (tester) async {
      await tester.pumpWidget(
        _harness(loginVm: loginVm, registerVm: registerVm),
      );
    ...
    testWidgets(
      'isSubmitting=true → кнопка disabled, AppLoader виден',
      (tester) async {
        await tester.pumpWidget(
          _harness(loginVm: loginVm, registerVm: registerVm),
        );

        registerNotifier.value = const RegisterViewState(isSubmitting: true);
        await tester.pump();
